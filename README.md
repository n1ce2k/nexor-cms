...
Модель пользователя остаётся в приложении. `nexor:install` дописывает её сам — спрашивает разрешение и добавляет контракт, трейты и колонки CMS в список заполняемых. Если разметка модели непривычная, команда покажет, что дописать руками:
