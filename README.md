...
Модель пользователя остаётся в приложении. `nexor:install` дописывает её сам — спрашивает разрешение и добавляет контракт, трейты, колонки CMS в список заполняемых и их приведения типов. Если разметка модели непривычная, команда покажет, что дописать руками:
...
Без ключа `nexor:install` не устанавливает CMS: ключ спрашивается первым шагом, для неинтерактивной установки — `--license-key=nxr-...`.

Ключ хранится в `NEXOR_LICENSE_KEY`. Если он истёк на работающем сайте, тот остаётся в редакции Lite и показывает плашку в панели; публичная часть не блокируется. `NEXOR_LICENSE` действует только в окружениях `local` и `testing`.
